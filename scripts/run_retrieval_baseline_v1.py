# scripts/run_retrieval_baseline_v1.py
...
PREDICTIONS_CSV = OUTPUT_DIR / "retrieval_baseline_predictions_v1.csv"
SUMMARY_CSV = OUTPUT_DIR / "retrieval_baseline_summary_v1.csv"
...
def main():
    print("=== RUN RETRIEVAL BASELINE V1 ===")
    ...
    for region in sorted(eval_meta["canonical_region"].unique()):
        ...
        if len(train_region_idx) == 0 or len(eval_region_idx) == 0:
            continue

        region_eval_feats = eval_feats[eval_region_idx]
        region_train_feats = train_feats[train_region_idx]

        sim = cosine_similarity(region_eval_feats, region_train_feats)
        best_train_local_idx = sim.argmax(axis=1)
    ...
    if pred_df.empty:
        raise ValueError("No retrieval predictions were generated.")
    ...
    sample_cols = [
        ...
        "cosine_similarity", "exact_match", "jaccard_similarity"
    ]
    ...
    print("Retrieval baseline completed successfully.")
